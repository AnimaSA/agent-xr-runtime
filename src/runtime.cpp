...
void Instance::InvalidateChildren()
{
	...
	for (XrSession sessionHandle : sessionCopy)
	{
		...
		Compositor* sessionCompositor = nullptr;
		{
			...
			sessionState->waitedFrameIds.clear();
			sessionState->begunFrameIds.clear();
			sessionState->RefreshFrameAliases();
...
void Session::RefreshFrameAliases()
{
	frameWaited = !waitedFrameIds.empty();
	waitedFrameId = frameWaited ? waitedFrameIds.front() : 0;
	frameBegun = !begunFrameIds.empty();
	begunFrameId = frameBegun ? begunFrameIds.front() : 0;
}
...
extern "C" AGENTXR_API XRAPI_ATTR XrResult XRAPI_CALL xrWaitFrame(XrSession session, const XrFrameWaitInfo* waitInfo, XrFrameState* frameState)
{
	return agentxr::GuardResult([&]() -> XrResult
	{
		...
		if (state.waitedFrameIds.size() >= agentxr::kMaxOutstandingFrames)
		{
			return XR_ERROR_LIMIT_REACHED;
		}
...
extern "C" AGENTXR_API XRAPI_ATTR XrResult XRAPI_CALL xrBeginFrame(XrSession session, const XrFrameBeginInfo* beginInfo)
{
	return agentxr::GuardResult([&]() -> XrResult
	{
		...
		agentxr::Session& state = *session->object;
		std::lock_guard lock(state.mutex);
		if (!state.running || (state.state != XR_SESSION_STATE_FOCUSED && state.state != XR_SESSION_STATE_STOPPING))
		{
			return XR_ERROR_CALL_ORDER_INVALID;
		}
		if (state.begunFrameIds.size() >= agentxr::kMaxOutstandingFrames)
		{
			return XR_ERROR_LIMIT_REACHED;
		}
		if (state.waitedFrameIds.empty())
		{
			return XR_ERROR_CALL_ORDER_INVALID;
		}
		const uint64_t begunFrameId = state.waitedFrameIds.front();
		...
		state.waitedFrameIds.pop_front();
		state.begunFrameIds.push_back(begunFrameId);
		state.RefreshFrameAliases();
...
extern "C" AGENTXR_API XRAPI_ATTR XrResult XRAPI_CALL xrEndFrame(XrSession session, const XrFrameEndInfo* endInfo)
{
	return agentxr::GuardResult([&]() -> XrResult
	{
		...
		if (state.begunFrameIds.empty() && state.waitedFrameIds.empty())
		{
			return XR_ERROR_CALL_ORDER_INVALID;
		}
		auto frameRecord = std::find_if(state.frames.begin(), state.frames.end(), [&](const agentxr::FrameRecord& record)
		{
			return record.displayTime == endInfo->displayTime && !record.ended && !record.discarded;
		});
		if (frameRecord == state.frames.end())
		{
			return XR_ERROR_TIME_INVALID;
		}
		const uint64_t frameId = frameRecord->id;
		auto begunIt = std::find(state.begunFrameIds.begin(), state.begunFrameIds.end(), frameId);
		auto waitedIt = std::find(state.waitedFrameIds.begin(), state.waitedFrameIds.end(), frameId);
		if (begunIt == state.begunFrameIds.end() && waitedIt == state.waitedFrameIds.end())
		{
			return XR_ERROR_TIME_INVALID;
		}
		const bool matchedWaited = begunIt == state.begunFrameIds.end();
		if (matchedWaited && state.begunFrameIds.empty())
		{
			return XR_ERROR_CALL_ORDER_INVALID;
		}
		...
		if (matchedWaited)
		{
			const uint64_t displacedId = state.begunFrameIds.front();
			auto displacedRecord = std::find_if(state.frames.begin(), state.frames.end(), [displacedId](const agentxr::FrameRecord& record)
			{
				return record.id == displacedId;
			});
			if (displacedRecord == state.frames.end())
			{
				return XR_ERROR_RUNTIME_FAILURE;
			}
			state.begunFrameIds.pop_front();
			state.waitedFrameIds.erase(waitedIt);
			displacedRecord->begun = false;
			state.waitedFrameIds.push_front(displacedId);
			frameRecord->begun = true;
		}
		else
		{
